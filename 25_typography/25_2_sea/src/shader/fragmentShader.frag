...
void main() {
    ...
    float si = sin((speed + vWeight.x + snoise(vWeight + time * 0.2) * 0.15) * 10.0) * 0.3;
    float t = (vWeight.y * 5.0 + si) * 1.0;

    vec2 uvForTex = getUVForTexture(uv, t);

    vec3 bgColor = vec3(0.0, 0.0, 0.6);
    ...
    float t2 = (1.0 - step(0.3, length(weight - vec2(0.5, 0.5))));
    if (t2 > 0.5) {
        t2 = snoise(vWeight * 1.0 + time * 0.05) * count;
        vec2 uvForTex2 = getUVForTexture(uv, t2);
        vec4 tex2 = texture2D(texture2, uvForTex2);
        color = mix(bgColor, txtColor, tex2.rgb);
        if (isOver) {
            color = bgColor;
        }
    }
