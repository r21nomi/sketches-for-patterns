...
void main() {
    ...
    // Moon
    vec2 weight2 = weight;
    weight2 -= vec2(0.0, 0.2);
    weight2 *= rotate(time * 0.5);
    float sunSize = 0.49;
    float t2 = 1.0 - step(sunSize, length(weight2));
    float l = map(mod((vWeight.x - vWeight.y) * 15.0 + time * 2.0, totalCount), 0.0, totalCount - 1.0, 1.0, totalCount - 1.0);
    t2 = mix(0.0, getTypePos(l), t2);
    ...
    for (float i = 0.0; i < 9.0; i++) {
        float speed = time * 1.3 + i;
        float si = sin(360.0 * vWeight.y * PI / 180.0 * 1.0 + speed + snoise(weight + i * 60.0) * 1.6) * 0.08;
