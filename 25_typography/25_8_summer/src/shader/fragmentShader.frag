...
void main() {
    ...
    // Sun
    vec2 weight2 = weight;
    weight2 -= vec2(0.0, 0.0);
    weight2 *= rotate(time * 0.5);
    float sunSize = 0.49;
    float t2 = 1.0 - step(sunSize, length(weight2));
    for (float i = 0.0; i < 360.0; i += 45.0) {
        weight2 *= rotate(i * PI / 180.0);
        t2 = max(t2, (1.0 - rect(vec2(sunSize + 0.3, 0.045), weight2 + snoise(weight2 + time * 0.5 + i) * 0.1)));
    }
    float l = mod(fract(length(weight2 * 5.0) - time * 0.9), count);
    if (l < 0.5) {
        l = 1.0;
    }
    t2 = mix(0.0, l, t2);

    vec2 uvForTex2 = getUVForTexture(uv, t2);
    vec4 tex2 = texture2D(texture2, uvForTex2);
    color = tex2.rgb;

    // Sea
    weight.y += 0.15;
    float t = getTypePos(0.0);
    for (float i = 0.0; i < 9.0; i++) {
        float speed = time * 3.0 + i;
        float si = sin(360.0 * vWeight.x * PI / 180.0 * 1.0 + speed + snoise(weight + i * 50.0)) * 0.08;
        t = mix(t, getTypePos(clamp(i, 1.0, 8.0)), 1.0 - step(0.1, length(weight.y + i / 9.0 + si)));
    }
    ...
    color = mix(color, tex.rgb, step(0.1, t));
