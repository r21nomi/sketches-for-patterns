...
float _(vec2 uv, float size, float boldness) {
    return rect(vec2(size), vec2(uv.x, uv.y * boldness));
}

float l(vec2 uv, float size, float boldness) {
    return rect(vec2(size), vec2(uv.x * boldness, uv.y));
}
