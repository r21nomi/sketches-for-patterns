...
class Tile {
    ...
    draw(shouldUpdate = false) {
        ...
        if (shouldUpdate) {
            // Update
            const dist = this.getDistanceFromScreenCenter();
    ...
    getDistanceFromScreenCenter() {
        ...
        const dx = centerOfScreen.x - centerOfTile.x;
        const dy = centerOfScreen.y - centerOfTile.y;
        return Math.sqrt(dx * dx + dy * dy) / Math.min(window.innerWidth, window.innerHeight);
    }
}
