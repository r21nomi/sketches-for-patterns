...
class Painter {
  ...
  constructor(_pos, _size, _shouldMove = true, _isSizeScalable = false) {
    ...
    this.speed = createVector(0.7, 0.7)
  ...
  update(min, max) {
    if (this.isSizeScalable) {
      let n = this.getNoiseValue(this.pos.x, this.pos.y)
      n = 1
    ...
    const vvv = 5
    const v = {
      x: this.speed.x * map(Math.cos((Math.cos(art.elapsedTime * 2.0 + n) + this.offset + n * 3.0) * 1.0), -1, 1, 1, vvv),
      y: this.speed.y * map(Math.sin((art.elapsedTime + this.offset) * 2.0 + n), -1, 1, 1, vvv)
    }
  ...
  draw(shouldUpdate = false) {
    ...
    if (shouldUpdate) {
      ...
      for (let j = 0; j < 4; j++) {
        ...
        const {  x, y, z, w, h } = this.getPositionAndSize(j)
        ...
        centerPosition.setXY(targetIndex, centerOfPainter.x, centerOfPainter.y)
        centerPosition.needsUpdate = true

        const rotationAttr = art.geometry.attributes.rotation
        rotationAttr.setX(targetIndex, rotation)
      ...
      for (let j = 0; j < 4; j++) {
        ...
        art.centerPositions.push(centerOfPainter.x, centerOfPainter.y)
        art.rotations.push(rotation)
      }
  ...
  getCalculatedRotation(x: number, y: number): number {
    const pos = {x, y}
    const _lastPos = !!this.lastScreenPos ? this.lastScreenPos : pos
    return Math.atan2(pos.y - _lastPos.y, pos.x - _lastPos.x)
  }
  getCenter() {
    if (art.isAxisCenter) {
      return {
   ...
   * @param vertexIndex
   */
  getPositionAndSize(vertexIndex: number) {
    let x, y, z
    let w, h
    ...
    if (art.isAxisCenter) {
      ...
      x = (vertexIndex === 0 || vertexIndex === 3) ? this.pos.x + PADDING : this.pos.x + this.size.x - PADDING
      y = (vertexIndex === 0 || vertexIndex === 1) ? this.pos.y + PADDING : this.pos.y + this.size.y - PADDING
    }
    z = this.pos.z
...
class Art {
  ...
  addMeshesToScenes() {
    ...
    this.geometry.setAttribute('centerPosition', new THREE.Float32BufferAttribute(this.centerPositions, 2))
  ...
  getInitialDrawerPosAndSize(index: number) {
    ...
    const size = createVector(basePainterSize.w, basePainterSize.h)

    if (this.isAxisCenter) {
      ...
      const center = createVector(
        -basePainterSize.w / 2 + (this.canvasSize.w / 2 * (Math.random() * 2 - 1)),
        -basePainterSize.h / 2 + (this.canvasSize.h / 2 * (Math.random() * 2 - 1))
      )
      return {
        pos: center,
        size: size,
        min: createVector(-this.canvasSize.w / 2, -this.canvasSize.h / 2),
        max: createVector(this.canvasSize.w / 2 - size.x, this.canvasSize.h / 2 - size.y)
      }
    }
  }
  getBasePainterSize() {
    ...
    const baseSize = Math.floor(Math.min(this.canvasSize.w, this.canvasSize.h) * 0.285)
