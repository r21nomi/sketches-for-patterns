...
class Painter {
  ...
  update(min, max) {
    ...
    const time = art.elapsedTime * map(Math.min(_speed.x, _speed.y), 0, 1, 0.1, 0.3)
    const v = {
      x: _speed.x * 2 + Math.cos(this.randValue + time + Math.cos(time + n * 0.1) * 3) * art.getWindowSize().w / 2,
      y: _speed.y * 2 + Math.sin(this.randValue + time + n * 0.1 + Math.sin(time) * 12) * art.getWindowSize().h / 2
    }
