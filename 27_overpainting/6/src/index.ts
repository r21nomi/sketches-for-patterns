...
class Painter {
  ...
  update(min, max) {
    if (this.isSizeScalable) {
      const speed = 0.8
      const n = map(this.getNoiseValue(art.elapsedTime * speed, art.elapsedTime * speed), 0, 1, 0.1, 1.3)
      this.size.x = this.originalSize.x * n
      this.size.y = this.originalSize.y * n
    }

    let n = this.getNoiseValue(art.elapsedTime * 1.5, art.elapsedTime)
    const _speed = 0.5
    const time = art.elapsedTime * map(Math.min(this.speed.x, this.speed.y), 0, 1, 0.1, 0.3) * 6
    const radius = Math.min(art.getWindowSize().w, art.getWindowSize().h)
    const v = {
      x: this.speed.x * _speed + Math.cos(this.randValue + time * _speed) * art.getWindowSize().h / 2.8,
      y: this.speed.x * _speed + Math.sin(this.randValue + time * _speed) * art.getWindowSize().h / 2.8
    }
...
class Art {
  ...
  async initialize() {
    ...
    canvas.style.backgroundColor = `#002fe5`
  ...
  createTexture(video): any {
    ...
    const _canvas: any = document.getElementById('canvas')
    _canvas.style.backgroundColor = `rgba(${rgba.r}, ${rgba.g}, ${rgba.b}, ${rgba.a})`
