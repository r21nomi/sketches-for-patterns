...
vec3 borderBtn(vec2 uv, vec3 color, vec3 bgColor, float size, float boldness) {
    ...
    color = mix(color, mix(bgColor * 0.9, bgColor * 0.0, uv.y * 20.0), b);
    return color;
}

vec3 btn(vec2 uv, vec3 color, vec3 bgColor, float size) {
    bgColor = mix(bgColor * 1.2, bgColor, smoothstep(0.0, 1.0, (uv.y + 0.45)));
    bgColor = mix(vec3(1.3), bgColor, smoothstep(0.0, 0.06, length(vec2(uv.x, uv.y * 1.0 + 0.04))));
    bgColor = mix(vec3(1.3), bgColor, smoothstep(0.0, 0.03, length(vec2(uv.x * 0.8, uv.y * 1.4 - 0.045))));
...
void main() {
    ...
    float windowBorderBoldness = 0.04;
