...
class L {
    constructor(id, pos, size, randomOffset, pointNum, fillColor, strokeColor) {
        this.id = id;
        this.pos = pos;
        this.size = size;
        this.pSize = 14;
    ...
    draw() {
        ...
        if (this.ps.length >= 4) {
            vertex(this.ps[0].pos.x, this.ps[0].pos.y);
        }
        noFill();
        endShape();
        ...
        for (let item of this.ps) {
            ...
            const sSize = this.pSize * max(min(this.id * 0.9, 2), 0.5);
            square(0, 0, sSize);
            noFill();
            square(0, 0, sSize * 1.5);
            if (this.id > 1) {
                square(0, 0, sSize * 2.0);
            }
