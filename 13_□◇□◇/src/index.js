...
const createMeshes = () => {
    ...
    const colors = [];
    const num = 60;
    uniforms.rect.uWidth.value = window.innerWidth / 2 * 0.9;  // offsetsは -1 ~ 1の値をとるので2で割る
    uniforms.rect.uHeight.value = window.innerHeight / 2 * 0.9;
    ...
    for (let x = 0; x < num; x++) {
        ...
        let padding = map(count, 0.0, num - 1.0, 1.0, 0.05);
        for (let j = 0; j < 4; j++) {
            index.push(count);
